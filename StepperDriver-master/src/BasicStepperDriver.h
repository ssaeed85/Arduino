...
protected:
    ...
    int dir_pin;
    int step_pin;
	int stepCount = 0;	//tracking step count. Along with revCount, gives us position of the motor
	int revCount = 0;	//tracking revolution count. Along with stepCount, gives us the position of the motor
...
public:
	...
	void moveAcc(long steps,int MAX_SPD, long angle);
    /*
     * Rotate the motor a given number of degrees (1-360)
     */
    void rotate(long deg);
	void rotateAcc(long deg, int MAX_SPD, long angle);
	...
	signed int getSteps();
	signed int getRevCount();
	void updateLoc(int steps);
	void calcStepPulse(int rpm);
	signed getRPM();
};
